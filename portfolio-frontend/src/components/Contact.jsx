...
import { Send, Loader, CheckCircle, AlertCircle, Mail, Phone, MapPin } from 'lucide-react';
...
const Contact = () => {
  ...
  useEffect(() => {
    // ⚠️ IMPORTANT: Remplacez par votre clé publique EmailJS
    // https://www.emailjs.com/docs/rest-api/
    emailjs.init('MS5HiWTONgye26QV3');
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await emailjs.send(
        'service_l35wsw7', // ⚠️ Remplacez par votre Service ID
        'template_g449lme', // ⚠️ Remplacez par votre Template ID
        {
          from_name: formData.name,
          from_email: formData.email,
          message: formData.message,
          to_email: 'keithsorail@gmail.com'
        }
      );
  ...
  const contactInfo = [
    {
      icon: <Mail size={24} />,
      title: 'Email',
      value: 'keithsorail@gmail.com',
      link: 'mailto:keithsorail@gmail.com'
    },
    {
      icon: <Phone size={24} />,
      title: 'Téléphone',
      value: '+224 624 284 874',
      link: 'tel:+224624284874'
    },
    {
      icon: <MapPin size={24} />,
      title: 'Localisation',
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {status === 'success' && (
                  <motion.div
                    ...
                    className="p-3 bg-green-500/20 border border-green-500/50 rounded-lg text-green-300 flex items-center gap-2 text-center justify-center"
                  >
                    <CheckCircle size={18} />
                ...
                {status === 'error' && (
                  <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="p-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-300 flex items-center gap-2 text-center justify-center"
                  >
                    <AlertCircle size={18} />
                    {error}
                  </motion.div>
